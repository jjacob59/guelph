...
saveRDS(x_all_encoded, file = paste0("~/vae_pseudo_", 1, ".Rdata"))
...
xte <- x_all_encoded 
g1 <- xte %>% ggplot(aes(x = V1, y = V2, color = type) ) +
  geom_point(size = 1, alpha = 0.4) +
  scale_colour_manual(values = c("pink", "blue", "green"))
g1

g2 <-  xte %>% ggplot(aes(x = V1, y = V2, color = type) ) +
  geom_point(size = 1, alpha = 0.4) +
  scale_color_manual( values = 
                        c("lightgreen", "pink", "blue", "red", "purple", "green", "yellow", "orange", "blue", "red"))

g2 

# <------ Panel B - just cells
clean <- xte %>% filter(!(class %in% c("Unknown", "H-junction", "P-junction", "Hyphae", "Pseudohyphae")))

g <- clean %>% ggplot(aes(x = V1, y = V2, color = class) ) +
  geom_point(size = 1, alpha = 0.4) +
  scale_color_manual( values = watlington())
g

# <--- Panel C - just white
white <- xte %>% filter(class %in% c("Yeast White", "Budding White"))

g <- white %>% ggplot(aes(x = V1, y = V2, color = class) ) +
  geom_point(size = 1) +
  scale_color_manual( values = watlington())
g

# <--- Panel D - components of varasana

just_val <- xte # %>% filter( type == "test")
g <- just_val %>% ggplot(aes(x = V1, y = V2, color = type) ) +
  geom_point(size = 1, alpha = 0.4) +
  scale_colour_manual(values = c("pink", "blue", "green"))
#  scale_color_manual( values = watlington())
g
