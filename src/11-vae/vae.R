...
xte <- x_all_encoded %>% add_column( tibble(dim1 = x$layout[,1], dim2 = x$layout[,2] ) )

g <- xte %>% ggplot(aes(x = dim1, y = dim2, color = class) ) +
  geom_point(size = 1) +
...
# display a 2D manifold of the cells
n <- 10  # figure with 10x10 cells
digit_size <- 128


# we will sample n points within [-4, 4] standard deviations
grid_x <- seq(5, 9, length.out = n)
grid_y <- seq(-2, -5, length.out = n)
...
for(i in 1:length(grid_x)){
  ...
  for(j in 1:length(grid_y)){
    ...
    column <- rbind(column, predict(generator, z_sample) %>% matrix(ncol = 128))
  }
